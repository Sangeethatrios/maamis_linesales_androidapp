...
//            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.P){
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                downloadID = downloadTask_Above_Android9(downloadURL,downloadFilename,context, Constants.FOLDER_EINVOICE);
            } else {
    ...
    public long downloadPdf(Object dataList, Context context){
        ...
        try {
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                downloadID = downloadTask_Above_Android9(downloadURL,downloadFilename,context, Constants.FOLDER_CASH_SUMMARY);
            } else {
                downloadID = downloadTask_Below_Android9(downloadURL,downloadFilename,context);
            }
    ...
    public long downloadVendorImage(Object dataList, Context context){
        ArrayList<Object> downloadData = (ArrayList<Object>) dataList;
        long downloadID = 0;
        ...
        String downloadCompanyID = (String) downloadData.get(0);
        String downloadVenderID = (String) downloadData.get(1);
        String downloadVendorURL = (String) downloadData.get(2);
        String downloadFilename = downloadVendorURL.split("/")[downloadVendorURL.split("/").length - 1];
        downloadFileName = downloadVendorURL.split("/")[downloadVendorURL.split("/").length - 1];
//        String Filename = downloadVendorURL.split("/")[downloadVendorURL.split("/").length - 1];

        Log.e("UPI_DOWNLOAD","downloadFileName : " + downloadFileName);
        try {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
                downloadID = downloadTask_Above_Android9(downloadVendorURL,downloadFilename,context, Constants.FOLDER_UPI_IMAGES);
            } else {
                downloadID = downloadTask_Below_Android9(downloadVendorURL,downloadFilename,context);
            }
    ...
    private long downloadTask_Above_Android9(String downloadURL, String downloadFileName, Context context, String folderName) throws Exception {
        long downloadId=0;
        if (!downloadURL.startsWith("http")) {
            return downloadId;
        }
        ...
        File destDir = null;
        try {
            File file = new File(context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS).toString() + File.separator + folderName);
            if (!file.exists()) {
            ...
            if (result.exists()) {
                boolean isDeleted =  result.delete();
